...
async function loadBets() {
  loading.value = true
  try {
  const { data, error } = await supabase.from('bets').select('*, creator_id').order('created_at', { ascending: false })
  ...
  <div style="max-width:900px;margin:2rem auto;padding:1rem">
    ...
    <ul v-else>
      <li v-for="b in bets" :key="b.id" style="margin-bottom:1rem;border:1px solid #eee;padding:1rem">
        <div><strong>{{ b.title }}</strong> — by {{ b.creator_email || b.creator_id || 'unknown' }}</div>
        <div>{{ b.description }}</div>
        <div>Amount: {{ b.amount || 0 }} — Status: {{ b.status || 'pending' }}</div>
      </li>
    </ul>
  </div>
</template>
